                    ...
                    <div class="b-b b-primary nav-active-primary">
                        <ul class="nav nav-tabs">
                            <li class="nav-item {{ empty($main_tab) || $main_tab == 'primary_info' ? 'active' : '' }}">
                                <a class="nav-link" href="#primary_info" data-toggle="tab" aria-expanded="false">Primary
                                    Info</a>
                            </li>
                            ...
                            <li class="nav-item {{ empty($main_tab) || $main_tab == 'consumers_face_details' ? 'active' : '' }}">
                                <a class="nav-link " href="#consumers_face_details" data-toggle="tab"
                                   aria-expanded="true">Consumers Face Details</a>
                            </li>
                            <li class="nav-item {{ empty($main_tab) || $main_tab == 'consumers_surgery_details' ? 'active' : '' }}">
                                <a class="nav-link" href="#consumers_surgery_details" data-toggle="tab"
                                   aria-expanded="false">Consumers Surgery Details</a>
                            </li>
                        </ul>
                    </div>
                    <div class="tab-content p-a m-b-md">
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'primary_info' ? 'active' : '' }}"
                             id="primary_info" aria-expanded="true">
                            @include('dashboard.consumers.tabs.personal_info')
                        </div>
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'multi_values_form_data' ? 'active' : '' }}"
                             id="multi_values_form_data" aria-expanded="true">
                            @include('dashboard.consumers.tabs.multi_values_form_data')
                        </div>
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'consumer_images' ? 'active' : '' }}"
                             id="consumer_images" aria-expanded="true">
                            @if($consumer->image->count() == 0)
                        ...
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'consumers_cards' ? 'active' : '' }}"
                             id="consumers_cards" aria-expanded="true">
                            @include('dashboard.consumers.tabs.consumer_cards_index')
                        </div>
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'consumers_face_details' ? 'active' : '' }}"
                             id="consumers_face_details" aria-expanded="true">
                            @include('dashboard.consumers.tabs.consumer_face_details_index')
                        </div>
                        <div class="tab-pane animated fadeIn text-muted {{ empty($main_tab) || $main_tab == 'consumers_surgery_details' ? 'active' : '' }}"
                             id="consumers_surgery_details" aria-expanded="true">
